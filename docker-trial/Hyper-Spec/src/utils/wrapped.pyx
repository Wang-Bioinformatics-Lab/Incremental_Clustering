...
cpdef list load_mgf_file(filename):
    ...
    cdef:
        ...
        int charge, spec_index
        object scan_py  # SCANS as str for "9_1" etc.; "-1" if missing
    ...
    while True:
        ...
        # elif(is_title(line)):
            ...
            # SCANS as string: support "9_1", "9_2" etc. (consensus samples)
            if read_len > 7:
                scan_py = PyBytes_FromStringAndSize(line + 6, read_len - 1 - 6).decode('utf-8').strip()
                if not scan_py:
                    scan_py = "-1"
            else:
                scan_py = "-1"
