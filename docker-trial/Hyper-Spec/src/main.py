...
def main(args: Union[str, List[str]] = None) -> int:
    ...
    # -------------------------------------------------------------------------
    # BUGFIX (cluster ID collision across charges): Each charge is clustered
    # separately with labels 0,1,2,...; without an offset, pd.concat merges
    # different charges into the same "cluster" (e.g. charge-2 and charge-3
    # both have cluster 0), causing huge precursor_mz spread within a cluster.
    # We add a per-charge offset so cluster IDs are globally unique.
    # REVERT: To restore original behavior, remove cluster_label_offset and
    # the "adjusted" logic; use cluster=list(cluster_labels_per_charge) in
    # assign() and delete the "Add offset" block.
    # -------------------------------------------------------------------------
    ...
    for prec_charge_i in tqdm.tqdm(config.cluster_charges):
        ...
        # Add offset to cluster labels so IDs are unique across charges.
        # Keep noise (-1) as -1; only shift non-negative labels.
        adjusted = [
            (l + cluster_label_offset) if l >= 0 else l
            for l in cluster_labels_per_charge
        ]
        mx = max(cluster_labels_per_charge)
        if mx >= 0:
            cluster_label_offset += int(mx) + 1
