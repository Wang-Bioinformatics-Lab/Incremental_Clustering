...
import pickle
import tempfile
# Use subprocess to run parsing in separate process that can be killed
class MPTimeoutError(Exception):
    pass
...
def fast_mgf_parse(filename, timeout_seconds=None):
    # Note: load_mgf_file doesn't support timeout_seconds parameter
    # Timeout is handled at Python level using multiprocessing
    read_spectra_list = load_mgf_file(filename)
    return read_spectra_list


def _parse_with_timeout(file, timeout_seconds):
    """Parse MGF file with timeout using subprocess to kill stuck C code"""
    if timeout_seconds is None or timeout_seconds <= 0:
        return fast_mgf_parse(file)
    
    # Use subprocess to run parsing in separate process that can be killed
    # This allows us to truly interrupt C code that's stuck
    result_file = os.path.join(tempfile.gettempdir(), f"mgf_parse_result_{os.getpid()}_{id(file)}.pkl")
    
    # Get the directory containing this file (hd_preprocess.py)
    current_dir = os.path.dirname(os.path.abspath(__file__))
    # Get the src directory (parent of current_dir if current_dir is src)
...
from hd_preprocess import fast_mgf_parse

file_path = r"{file}"
result_file = r"{result_file}"
try:
    result = fast_mgf_parse(file_path)
...
'''
    ...
    try:
        ...
        try:
            ...
            import logging
            logging.warning(f"Killing stuck subprocess for file: {os.path.basename(file)}")
            process.kill()
            process.wait()
            raise MPTimeoutError(f"Timeout ({timeout_seconds}s) while parsing file: {os.path.basename(file)}")
        ...
        if os.path.exists(result_file):
            ...
            if status == "success":
                ...
                raise RuntimeError(f"Error parsing file: {data}")
        else:
            raise RuntimeError(f"No result file created for file: {os.path.basename(file)}")
        ...
        if os.path.exists(result_file):
            try:
                ...
                pass
        # If subprocess fails, fall back to direct call
        import logging
        logging.debug(f"Subprocess failed, using direct call: {e}")
        start_time = time.time()
        result = fast_mgf_parse(file)
        elapsed = time.time() - start_time
        if elapsed > timeout_seconds:
            import logging
            logging.warning(f"Timeout ({elapsed:.2f}s > {timeout_seconds}s) while parsing file: {os.path.basename(file)}")
            raise MPTimeoutError(f"Timeout ({timeout_seconds}s) while parsing file: {os.path.basename(file)}")
        return result


def _preprocess_with_timeout(spec_list, timeout_seconds, **kwargs):
    """Preprocess spectra with timeout"""
    if timeout_seconds is None or timeout_seconds <= 0:
        return preprocess_read_spectra_list(spectra_list=spec_list, **kwargs)
    
    # For preprocessing, use direct call with timeout check
    # Preprocessing is Python code, so it's less likely to get stuck in C
    start_time = time.time()
    result = preprocess_read_spectra_list(spectra_list=spec_list, **kwargs)
    elapsed = time.time() - start_time
    if elapsed > timeout_seconds:
        import logging
        logging.warning(f"Timeout ({elapsed:.2f}s > {timeout_seconds}s) while preprocessing")
        raise MPTimeoutError(f"Timeout ({timeout_seconds}s) while preprocessing")
    return result
...
def load_process_single(
    ...
    """
    Load and process a single MGF file with timeout protection.
    
    Returns:
        spec_list: List of spectra, or None if timeout occurred
    """
    start_time = time.time()
    
    try:
        # Parse file with timeout (using multiprocessing to kill stuck processes)
        try:
            spec_list = _parse_with_timeout(file, timeout_seconds)
        except MPTimeoutError:
            import logging
            logging.warning(f"Timeout ({timeout_seconds}s) while parsing file: {os.path.basename(file)}")
            return None
        
        # Check if parsing returned empty list
        if not spec_list or len(spec_list) == 0:
            import logging
            logging.warning(f"Empty result after parsing file: {os.path.basename(file)}")
            return None
        
        if if_preprocess:
            # Calculate remaining time for preprocessing
            elapsed = time.time() - start_time
            remaining_time = timeout_seconds - elapsed if timeout_seconds else None
            
            # Preprocess spectra with timeout
            try:
                spec_list = _preprocess_with_timeout(
                    spec_list,
                    remaining_time,
                    min_peaks=min_peaks,
                ...
                import logging
                elapsed = time.time() - start_time
                logging.warning(f"Timeout ({timeout_seconds}s) while preprocessing file: {os.path.basename(file)} (elapsed: {elapsed:.2f}s)")
                return None
            
            # Check if preprocessing returned empty list
            if not spec_list or len(spec_list) == 0:
                import logging
                logging.warning(f"Empty result after preprocessing file: {os.path.basename(file)}")
                return None

        return spec_list
    except Exception as e:
        import logging
        logging.error(f"Error processing file {os.path.basename(file)}: {str(e)}")
        import traceback
...
def load_process_spectra_parallel(
    ...
    with Parallel(n_jobs=config.cpu_core_preprocess) as parallel_pool:
        ...
        # Filter out None results (timeout or error) and track skipped files
        read_spectra_list = []
        for i, (result, file_path) in enumerate(zip(results, input_files)):
            if result is None:
                skipped_files.append(file_path)
                logger.warning(f"Skipped file {i+1}/{len(input_files)} (timeout or error): {os.path.basename(file_path)}")
            elif isinstance(result, list) and len(result) == 0:
                skipped_files.append(file_path)
                logger.warning(f"Skipped file {i+1}/{len(input_files)} (empty result): {os.path.basename(file_path)}")
            else:
                read_spectra_list.append(result)
        
        logger.info(f"Successfully processed {len(read_spectra_list)}/{len(input_files)} files")
    
    if skipped_files:
        logger.warning(f"Total {len(skipped_files)} files skipped due to timeout or error:")
        for f in skipped_files:
            logger.warning(f"  - {os.path.basename(f)}")
    
    if not read_spectra_list:
        error_msg = f"No files were successfully processed! All {len(input_files)} files timed out or had errors."
        if skipped_files:
            error_msg += f"\nSkipped files: {len(skipped_files)}"
            error_msg += f"\nFirst few skipped files: {[os.path.basename(f) for f in skipped_files[:5]]}"
    ...
    for c in spectra_meta_df.columns:
        ...
        # scan: keep as-is (str for "9_1" etc., or int); no cast to int32
...
def load_raw_spectra_parallel(
    ...
    # load_process_single can return None (timeout, empty, or exception); treat as no spectra
    none_count = sum(1 for i in read_spectra_list if i is None)
    if none_count:
        logger.warning('{} file(s) returned no spectra (timeout/empty/error) and were skipped'.format(none_count))
    read_spectra_list = [j for i in read_spectra_list if i is not None for j in i]
    ...
    for c in read_spectra_list.columns:
        ...
        # scan: keep as-is (str for "9_1" etc.)
